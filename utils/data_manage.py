...
def dataset_prepare(mat: dict, dtype: torch.dtype = torch.complex128, device: str = 'cuda', batch_size: OptionalInt = None, 
    ...
    input = mat['TXa'].reshape(-1)
    target = mat['RXa'].reshape(-1)
    nf = mat['NFa'].reshape(-1)

    if delay_d is not None and delay_d != 0:
        target = np.roll(target, -delay_d)
        nf = np.roll(nf, -delay_d)

    input = torch.tensor(input, dtype=dtype).view(1, 1, -1).to(device)
    target = torch.tensor(target, dtype=dtype).view(1, 1, -1).to(device)
    nf = torch.tensor(nf, dtype=dtype).view(1, 1, -1).to(device)

    scale =  1 / input.abs().max()
    input *= scale
    target *= scale
    nf *= scale
