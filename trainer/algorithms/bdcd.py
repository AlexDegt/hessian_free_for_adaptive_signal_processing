...
def train_coordinate_descent(model: nn.Module, train_dataset: DataLoaderType, validate_dataset: DataLoaderType, 
    ...
    """
    Function implements dichotomous coordinate descent (DCD) method using mixed hessian for holomorphic functions. 
    ...
    # while grad_norm is None or grad_norm >= min_grad_norm:
        ...
        for j, batch in enumerate(train_dataset):
            ...
            with torch.no_grad():
                ...
                hess = lam * hess + delta_hess
                grad = lam * grad + 2 * delta_grad
                del delta_hess, delta_grad
                torch.cuda.empty_cache()

            # Store parameters
            curr_params = SICOracle.get_flat_params(name_list=weight_names)

            for _ in range(iter_num):
                nr = grad.real.abs().argmax()
                ni = grad.imag.abs().argmax()
                if grad[nr].real.abs() > grad[ni].imag.abs():
                    s = 1
                    n = nr
                else:
                    s = 1j
                ...
                m = 0
                alpha = alpha_init
                while rtmp.abs() <= alpha * hess[n, n].real:
                    m = m + 1
                    alpha = alpha / 2
                    if m > precision:
                        break
                step = torch.zeros_like(curr_params)
                step[n] = rtmp.sign() * alpha * s
                SICOracle.set_flat_params(SICOracle.get_flat_params(name_list=weight_names) - step, name_list=weight_names)
                grad -= 2 * rtmp.sign() * alpha * hess[:, n] * s
