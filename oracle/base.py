...
class Oracle(object):
    ...
    @torch.no_grad()
    def add_flat_params(self, flat_params: Tensor, name_list: StrOrList = None, idxs: OptionalTensor = None) -> None:
        """
        Use this method to add 1d vector to the model parameters
        
        Args:
            flat_params (Tensor): 1d float Tensor used to store model parameters, which implied to be added.
            name_list (str or list of str, optional): By spceifying `name_list` it is possible to set add 1d vector to 
                several named parameters. It is strictly advised to have a similar order of named parameters used in
                get_flat_params method due to model consistency reasons. Defaults to "None".
            idxs (Tensor, optional): 1d int Tensor of indexes to perform masked update only for specified indexes in
                flat_params. If idxs is not None we consider flat_params as a 1d float Tensor of size len(idxs) with elements
                corresponding to indexes from idxs representing positions in the original 1d flattened vector of model
        ...
        """
        if not (idxs is None):
            params_zeros = torch.zeros_like(self.get_flat_params(detach=True, name_list=name_list))
            params_zeros[idxs] = flat_params
            flat_params = params_zeros
        offset = 0
        if name_list is None:
            for p in self._model.parameters():
                numel = p.numel()
                # view as to avoid deprecated pointwise semantics
                p.data = p + flat_params[offset:offset + numel].view_as(p)
                offset += numel
        else:
            if isinstance(name_list, str):
                name_list = [name_list]
    ...
    def direction_through_jacobian(self, signal_batch: BatchType, batch_to_tensors: BatchTensorType,
        ...
        model_output = self._model(signal_batch_input)

        error_vec = torch.permute(model_output - signal_batch_output, (0, 2, 1))
